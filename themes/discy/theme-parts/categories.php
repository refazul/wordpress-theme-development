...
if (!empty($terms) && !is_wp_error($terms)) {
	$term_list = '<div class="row row-warp cats-sections cat_'.$cat_style.'">';
		foreach ($terms as $term) {
			...
			$tax_col = "col6";
			if (($cats_tax == 'question' || $cats_tax == wpqa_question_categories) && ($cat_style == "with_icon" || $cat_style == "icon_color" || $cat_style == "with_cover_1" || $cat_style == "with_cover_2" || $cat_style == "with_cover_3")) {
				$tax_col = "col4";
				if ($theme_sidebar == "full") {
					$tax_col = "col3";
				}
			}else if ($theme_sidebar == "full" && ($cats_tax == 'question' || $cats_tax == wpqa_question_categories) && ($cat_style == "with_cover_4" || $cat_style == "with_cover_5" || $cat_style == "with_cover_6")) {
				$tax_col = "col4";
			}else if (($cats_tax == 'question' || $cats_tax == wpqa_question_categories) && ($cat_style == "with_icon_1" || $cat_style == "with_icon_2" || $cat_style == "with_icon_3" || $cat_style == "with_icon_4")) {
				$tax_col = "col4";
				if ($theme_sidebar == "full") {
					$tax_col = "col3";
				}
