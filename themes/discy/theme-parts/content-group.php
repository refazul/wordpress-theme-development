...
<div class="content_group_item">
			...
			<div class="header-info">
				<div class="title">
					<h3>
						...
						<a href="<?php echo get_author_posts_url($post_author);?>" title="<?php echo esc_attr($display_name);?>"><?php echo esc_html($display_name);?></a>
					...
					<div class="posts-action">
						...
						if (($post_data->post_status == 'publish' && $posts_delete == "on" && $post_author == $user_id) || (isset($edit_delete_posts_comments["delete"]) && $edit_delete_posts_comments["delete"] == "delete" && isset($allow_group_moderators)) || $is_super_admin) {?>
							<a class="posts-delete" href="<?php echo esc_url_raw(add_query_arg(array("activate_delete" => true,"delete" => $post_id,"wpqa_delete_nonce" => wp_create_nonce("wpqa_delete_nonce")),wpqa_custom_permalink($post_id,"view_posts_group","view_group_post")))?>"><i class="icon-trash"></i><?php esc_html_e("Delete","discy")?></a>
						<?php }?>
					</div>
				</div>
	...
	if (isset($show_group_footer)) {?>
		<footer class="question-footer posts-footer">
			...
			if ($post_data->post_status == 'publish') {
				if ((is_user_logged_in() && $group_comments == "on") || isset($allow_group_moderators) || $is_super_admin) {?>
					<a class="meta-answer meta-comment-a meta-group-comments<?php echo (!wpqa_is_view_posts_group() && $editor_group_post_comments == "on"?"":" meta-group-comments-ajax")?>" href="<?php echo wpqa_custom_permalink($post_id,"view_posts_group","view_group_post")?>#respond"><?php esc_html_e("Comment","discy")?></a>
				<?php }
			}else if (is_user_logged_in() && ($is_super_admin || (isset($edit_delete_posts_comments["edit"]) && $edit_delete_posts_comments["edit"] == "edit" && isset($allow_group_moderators)))) {
	...
	<div class="embed_comments">
		...
		if (is_array($comments_args) && !empty($comments_args)) {
			...
			<ol<?php echo (wpqa_is_view_posts_group()?' id="group-comments"':"")?> class="commentlist clearfix">
				<?php wp_list_comments(array("callback" => "wpqa_comment","comment_type" => "comment_group"),$comments_args);?>
				</li>
			</ol>
