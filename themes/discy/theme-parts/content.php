...
if ('post' !== $post_data->post_type || ('post' === $post_data->post_type && ($custom_permission != "on" || $is_super_admin || (is_user_logged_in() && isset($roles["show_post"]) && $roles["show_post"] == 1) || (!is_user_logged_in() && $show_post == "on") || ($user_id > 0 && $user_id == $post_author)))) {
	if (isset($blog_h) && $blog_h == "blog_h") {?>
		<div id="post-<?php echo (int)$post_id?>" <?php post_class('article-post article-post-only clearfix'.$featured_style.($post_data->post_content == " post-no-content"?" post--content":"").($post_style == "style_2"?" post-style-2":"").($post_style == "style_3"?" post-style-3 post-with-columns".$post_columns.($masonry_style == "on"?" post-masonry":""):""));?>>
	<?php }else {?>
		<article id="post-<?php echo (int)$post_id?>" <?php post_class('article-post article-post-only clearfix'.$featured_style.($post_data->post_content == " post-no-content"?" post--content":"").($post_style == "style_2"?" post-style-2":"").($post_style == "style_3"?" post-style-3 post-with-columns".$post_columns.($masonry_style == "on"?" post-masonry":""):""));?>>
		...
		<div class="single-inner-content">
					...
					if (isset($sort_meta_title_image_value["value"]) && $sort_meta_title_image_value["value"] == "image") {
						...
						<header class="article-header<?php echo (((empty($blog_h) && isset($wp_page_template) && ($wp_page_template == "template-blog.php" || $wp_page_template == "template-home.php")) || !is_page()) && !is_attachment() && $author_by == "on"?"":" header-no-author").($author_by != "on" && $post_date != "on" && $title_post != "on" && $category_post != "on" && $post_comment != "on" && $post_views != "on"?" header-no-meta":"")?>">
							...
							if ( ((empty($blog_h) && isset($wp_page_template) && ($wp_page_template == "template-blog.php" || $wp_page_template == "template-home.php")) || !is_page()) && !is_attachment() && $author_by == "on" ) {
								if ($post_author > 0) {
									echo sprintf(esc_html_x( '%s', 'post author', 'discy' ),'<a class="post-author" rel="author" href="' . esc_url( get_author_posts_url( $post_author ) ) . '">'.esc_html(get_the_author()).'</a>');
								}else {
			...
			<div class="post-wrap-content<?php echo ((is_page() && empty($blog_h) && (empty($wp_page_template) || ($wp_page_template != "template-blog.php" && $wp_page_template != "template-home.php")))?"":" post-content ").(is_attachment()?" post-attachment ":"")?>">
				...
				if ( (is_single() && empty($blog_h)) || (is_page() && empty($blog_h) && (empty($wp_page_template) || ($wp_page_template != "template-blog.php" && $wp_page_template != "template-home.php"))) ) {
					...
					if ( $post_tags == "on" && 'post' === $post_data->post_type ) {
							...
							echo '<div class="tagcloud">';
								...
								foreach ($terms as $term) :
									if (isset($term->slug) && isset($term->name)) {
										$get_term_link = get_term_link($term->slug, 'post_tag');
				...
				do_action("wpqa_after_post_tags",$post_id,$post);?>
			</div>
				...
				<footer<?php echo ($pending_posts?" class='pending-post-footer'":"")?>>
					...
					if (has_wpqa() && $pending_posts) {
						...
						if ( (strpos(get_the_content(),'more-link') !== false || $read_more == "on") && !is_single() && (empty($blog_h) && (isset($wp_page_template) && ($wp_page_template == "template-blog.php" || $wp_page_template == "template-home.php")) || !is_page()) ) {?>
							<a<?php echo esc_attr($post_link_target)?> class="post-read-more" href="<?php echo esc_url(has_wpqa() && $post_data->post_type == "posts"?wpqa_custom_permalink($post_id,"view_posts_group","view_group_post"):get_permalink($post_id))?>" rel="bookmark" title="<?php esc_attr_e('Read','discy')?> <?php the_title()?>"><?php esc_html_e('Read more','discy')?></a>
						<?php }
	...
	echo '<article class="private-post article-post clearfix">
		<div class="alert-message error"><i class="icon-cancel"></i><p>'.esc_html__("Sorry, you do not have permission to view posts.","discy").'</p></div>
