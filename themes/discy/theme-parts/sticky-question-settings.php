<?php if (isset($sticky_questions) && is_array($sticky_questions) && !empty($sticky_questions) && $paged == 1) {
	...
	$custom_show_sticky = apply_filters('discy_show_sticky',false);
	...
	if ((isset($wp_page_template) && $wp_page_template == "template-question.php" && isset($orderby_post) && $orderby_post == "polls") || (isset($_GET["type"]) && $_GET["type"] == "poll") || $custom_show_sticky == true || (isset($first_one) && ($first_one == "polls" || $first_one == "polls-time" || $first_one == "poll-feed" || $first_one == "poll-feed-time")) || (isset($_GET["show"]) && ($_GET["show"] == "polls" || $_GET["show"] == "polls-time" || $_GET["show"] == "poll-feed" || $_GET["show"] == "poll-feed-time"))) {
		$sticky_args = array_merge($custom_args,$author__not_in,array("nopaging" => true,"post_type" => "question","post__in" => $questions_sticky_isset,"meta_query" => array(array('relation' => 'AND',array("key" => "question_poll","value" => "on","compare" => "LIKE"),$query_sticky_meta))));
		$custom_show_sticky = apply_filters('discy_sticky_args',$sticky_args,$custom_args,$questions_sticky_isset,$query_sticky_meta);
	}else {
		$sticky_args = array_merge($custom_args,$author__not_in,array("nopaging" => true,"post_type" => "question","post__in" => $questions_sticky_isset,"meta_query" => $query_sticky_meta));
	}
