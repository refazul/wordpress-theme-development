...
include locate_template("includes/".(isset($its_question) && ("question" == $its_question || "asked-question" == $its_question)?"question":"loop")."-setting.php");
...
if (empty($blog_h) && ((((isset($tab_category) && $tab_category == true) || (isset($tab_tag) && $tab_tag == true)) && ($first_one == $answers_slug || $first_one == $answers_slug_2))) || (isset($wp_page_template) && (($wp_page_template == "template-home.php" && is_user_logged_in() && isset($first_one) && $first_one != "" && ($first_one == $answers_might_like_slug || $first_one == $answers_for_you_slug || $first_one == $answers_might_like_slug_2 || $first_one == $answers_for_you_slug_2))) || (isset($wp_page_template) && (($wp_page_template == "template-home.php" && isset($first_one) && $first_one != "" && ($first_one == $answers_slug || $first_one == $answers_slug_2 || (!is_user_logged_in() && ($first_one == $answers_might_like_slug || $first_one == $answers_for_you_slug || $first_one == $answers_might_like_slug_2 || $first_one == $answers_for_you_slug_2)))) || $wp_page_template == "template-comments.php")))) {
	...
	if ((((isset($tab_category) && $tab_category == true) || (isset($tab_tag) && $tab_tag == true)) && ($first_one == $answers_slug || $first_one == $answers_slug_2)) || (isset($wp_page_template) && ($wp_page_template == "template-home.php" && is_user_logged_in() && isset($first_one) && $first_one != "" && ($first_one == $answers_might_like_slug || $first_one == $answers_for_you_slug || $first_one == $answers_might_like_slug_2 || $first_one == $answers_for_you_slug_2)))) {
		...
		$cat_posts = get_objects_in_term($category_list,$get_tax_name);
	...
	if ($orderby_answers == 'votes' && ($post_type == 'question' || $post_type == 'asked-question')) {
	...
	if (!empty($comments_all)) {
		if (isset($post_pagination) && ($post_pagination == "pagination" || $post_pagination == "standard")) {
			$pagination_args = array(
				...
				'next_text' => ($post_pagination == 'standard'?'<i class="icon-left-thin"></i><span>'.esc_html__('Old Answers',"discy").'</span>':'<i class="icon-right-open"></i>'),
			);
	...
	<?php }else {
		echo '<div class="alert-message warning"><i class="icon-flag"></i><p>'.($post_type == 'question' || $post_type == 'asked-question'?esc_html__("There are no answers yet.","discy"):esc_html__("There are no comments yet.","discy")).'</p></div>';
	}
	...
	<?php }else if (isset($post_pagination) && ($post_pagination == "infinite_scroll" || $post_pagination == "load_more")) {
		...
		if (has_wpqa()) {
			wpqa_load_pagination(array(
				...
				"its_post_type" => (isset($its_question)?$its_question:false),
	...
	if (isset($blog_h) && $blog_h == "blog_h") {
		...
		if ($loop_query == true) {
		...
		}else if (((((isset($tab_category) && $tab_category == true)) || (isset($tab_tag) && $tab_tag == true)) || (isset($wp_page_template) && ($wp_page_template == "template-question.php" || $wp_page_template == "template-home.php"))) && isset($orderby_post) && ($orderby_post == "popular" || $orderby_post == "most_visited" || $orderby_post == "most_voted")) {
			$active_sticky = false;
		...
		}else if (isset($wp_page_template) && $wp_page_template == "template-home.php" && isset($first_one) && $first_one != "" && is_string($first_one) && isset($get_tax->term_id) && is_numeric($get_tax->term_id) && $get_tax->term_id > 0) {
			$active_sticky = $show_sticky = $post_not_true = true;
			$custom_args   = array("post_type" => "question","tax_query" => array(array("taxonomy" => wpqa_question_categories,"field" => "id","terms" => $get_tax->term_id)));
			$array_data    = array_merge($orderby_array,$cats_post,array("post_type" => $post_type,"ignore_sticky_posts" => 1,"paged" => $paged,"posts_per_page" => $post_number,"tax_query" => array(array("taxonomy" => wpqa_question_categories,"field" => "slug","terms" => $first_one))));
			...
			if (is_array($following_questions_user) && !empty($following_questions_user) && count($following_questions_user) > 0) {
				$array_data = array_merge($custom_category,$specific_date_array,array("post_type" => "question","paged" => $paged,"post__in" => $following_questions_user));
			}
			...
			$active_sticky = $show_sticky = $post_not_true = false;
			$no_favorites  = true;
			$old_themes    = discy_options("old_themes");
			...
			if (is_array($_favorites) && !empty($_favorites) && count($_favorites) > 0) {
				$array_data = array_merge($custom_category,$specific_date_array,array("post_type" => "question","paged" => $paged,"post__in" => $_favorites));
			}
		}else {
			$post_not_true = true;
			if (($first_one == $feed_slug && is_user_logged_in()) || ($first_one == $feed_slug_2 && is_user_logged_in()) || ($first_one == $poll_feed_slug && is_user_logged_in()) || ($first_one == $poll_feed_slug_2 && is_user_logged_in()) || (isset($wp_page_template) && $wp_page_template == "template-question.php" && $orderby_post == "feed" && is_user_logged_in())) {
				...
				if ($user_following_if == "yes" && $cat_following_if == "yes" && $tag_following_if == "yes" && ($all_following != "" || $user_following != "")) {
					...
					AND $wpdb->posts.post_type = 'question'
					...
					if (is_array($query) && !empty($query)) :?>
						<div class="post-articles<?php echo (isset($its_question) && ("question" == $its_question || "asked-question" == $its_question)?" question-articles".(isset($question_columns) && $question_columns == "style_2" && isset($count_total) && $count_total > 0?" row row-boot row-warp".(isset($masonry_style) && $masonry_style == "on" && isset($count_total) && $count_total > 0?" isotope":""):""):"").($post_pagination == "none"?" no-pagination":"").(isset($blog_h) && $blog_h == "blog_h"?" post-articles-blog-h":"").(isset($post_style) && $post_style == "style_3"?" row row-boot row-warp":"")?>">
							<?php foreach ($query as $post) {
								setup_postdata($post->ID);
								$k_ad_p++;
								include locate_template("theme-parts/content".(isset($its_question) && ("question" == $its_question || "asked-question" == $its_question)?"-question":"").".php");
							}?>
						</div>
					<?php else :
				...
				if ($show_login == "login" && (($first_one == $feed_slug && !is_user_logged_in()) || ($first_one == $feed_slug_2 && !is_user_logged_in()) || ($first_one == $poll_feed_slug && !is_user_logged_in()) || ($first_one == $poll_feed_slug_2 && !is_user_logged_in()) || (isset($wp_page_template) && $wp_page_template == "template-question.php" && $orderby_post == "feed" && !is_user_logged_in()))) {
		...
		$poll_array = apply_filters("discy_poll_array",$poll_array);
		$post_not_true = true;
		...
		$post_not_true = true;
		$array_data = apply_filters("discy_args_question_category",array_merge($question_numbers,array("ignore_sticky_posts" => 1,"post_type" => "question","paged" => $paged,"tax_query" => array(array("taxonomy" => $tax_question,"field" => "id","terms" => $category_id,'operator' => 'IN')))),$tax_question,$paged,$question_numbers,$category_id);
	}else if (is_tax(wpqa_question_tags)) {
		$post_not_true = true;
		$array_data = array_merge(array("ignore_sticky_posts" => 1,"post_type" => "question","paged" => $paged,"tax_query" => array(array("taxonomy" => wpqa_question_tags,"field" => "id","terms" => $category_id,"operator" => "IN"))));
	}else if ($last_one == "questions" || $last_one == "posts") {
		$array_data = array("author" => $get_user_var,"post_type" => ($last_one == "questions"?"question":"post"),"paged" => $paged,"ignore_sticky_posts" => 1);
	}else if ($last_one == "polls") {
		$array_data = array("author" => $get_user_var,"post_type" => "question","paged" => $paged,"meta_query" => array(array("key" => "question_poll","value" => "on","compare" => "=")));
		...
		if (is_array($_favorites) && !empty($_favorites) && count($_favorites) > 0) {
			$array_data = array("post_type" => "question","paged" => $paged,"post__in" => $_favorites);
		}
		...
		$array_data = array("post_type" => "asked-question","paged" => $paged,"meta_query" => array(array_merge(array($meta_asked),array(array("type" => "numeric","key" => "user_id","value" => (int)$get_user_var,"compare" => "=")))));
	}else if ($pay_ask == "on" && ($last_one == "paid-questions")) {
		$array_data = array("author" => $get_user_var,"post_type" => "question","paged" => $paged,"meta_query" => array(array('type' => 'numeric',"key" => "_paid_question","value" => 'paid',"compare" => "=")));
	}else if ($last_one == "followed") {
		$following_questions_user = get_user_meta($get_user_var,"following_questions",true);
		if (is_array($following_questions_user) && !empty($following_questions_user) && count($following_questions_user) > 0) {
			$array_data = array("post_type" => "question","paged" => $paged,"post__in" => $following_questions_user);
		}
		...
		if (is_array($following_me) && count($following_me) > 0) {
			$array_data = array("post_type" => ($last_one == "followers-questions"?"question":"post"),"paged" => $paged,"author__in" => $following_me,"ignore_sticky_posts" => 1);
		}
		...
		$post_type_pending = array("question","asked-question");
		...
		if (!$is_super_admin) {
			if (((is_string($post_type_pending) && ($post_type_pending == "question" || $post_type_pending == "asked-question")) || (is_array($post_type_pending) && (in_array("question",$post_type_pending) || in_array("asked-question",$post_type_pending)))) && !in_array("q-0",$moderator_categories)) {
	...
	$all_sticky_posts = apply_filters("discy_sticky_posts",get_option('sticky_posts'));
	...
	<section<?php echo ((isset($post_style) && $post_style == "style_3") || (isset($question_columns) && $question_columns == "style_2")?" class='section-post-with-columns'":"")?><?php echo ($last_one == "questions" || $last_one == "asked" || $last_one == "asked-questions" || $last_one == "paid-questions" || $last_one == "polls" || $last_one == "followed" || $last_one == "favorites" || $last_one == "followers-questions" || $last_one == "posts" || $last_one == "followers-posts" || (has_wpqa() && (wpqa_is_pending_questions() || wpqa_is_pending_posts()))?" id='section-".wpqa_user_title()."'":"")?>>
		<?php if ((isset($no_favorites) || $last_one == "favorites") && empty($_favorites)) {
			...
			if (isset($its_question) && ("question" == $its_question || "asked-question" == $its_question)) {
				$page_tamplate = true;
			}
			...
			if (!isset($custom_sql)) :
				if (!empty($sticky_questions) || !empty($all_sticky_posts) || isset($custom_args) || (isset($query_wp) && $query_wp->have_posts()) || (have_posts() && empty($array_data))) :
					if (isset($blog_h) || empty($wp_page_template) || (isset($wp_page_template) && $wp_page_template != "template-users.php" && $wp_page_template != "template-contact.php" && $wp_page_template != "template-faqs.php" && $wp_page_template != "template-categories.php" && $wp_page_template != "template-tags.php")) :
						...
						<h2 class="screen-reader-text"><?php echo esc_html(get_bloginfo('name','display'))." ".esc_html__("Latest","discy")." ";printf("%s",(isset($its_question) && ("question" == $its_question || "asked-question" == $its_question)?esc_html__("Questions","discy"):esc_html__("Articles","discy")))?></h2>
						<div class="post-articles<?php echo (isset($its_question) && ("question" == $its_question || "asked-question" == $its_question)?" question-articles".(isset($question_columns) && $question_columns == "style_2" && isset($count_total) && $count_total > 0?" row row-boot row-warp".(isset($masonry_style) && $masonry_style == "on" && isset($count_total) && $count_total > 0?" isotope":""):""):"").($post_pagination == "none"?" no-pagination":"").(isset($blog_h) && $blog_h == "blog_h"?" post-articles-blog-h":"").(empty($more_link)?" articles-no-pagination":"").(isset($post_style) && $post_style == "style_3"?" row row-boot row-warp":"")?>">
					...
					if (!isset($no_feed_questions)) {
						if ((isset($blog_h) && $blog_h == "blog_h") || (isset($show_custom_error) && $show_custom_error == true && (!isset($is_questions_sticky) || (isset($is_questions_sticky) && $is_questions_sticky != true)))) {
							echo "<div class='alert-message warning'><i class='icon-flag'></i><p>".(isset($its_question) && ($its_question == "question" || $its_question == "asked-question")?esc_html__("There are no questions yet.","discy"):esc_html__("There are no posts yet.","discy"))."</p></div>";
						}else {
