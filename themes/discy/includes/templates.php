<?php if (isset($blog_h) && $blog_h == "blog_h") {
	...
	$orderby_post            = discy_options("orderby_post_h");
	$order_post              = discy_options("order_post_h");
	$post_display            = discy_options("post_display_h");
	$post_single_category    = discy_options("post_single_category_h");
	$post_categories         = discy_options("post_categories_h");
	$post_exclude_categories = discy_options("post_exclude_categories_h");
	$post_posts              = discy_options("post_posts_h");
	...
	if (isset($first_one) && $first_one != "") {
		if ((isset($tab_category) && $tab_category == true) || (isset($tab_tag) && $tab_tag == true)) {
			...
			if ($first_one == $recent_questions_slug_2) {
				$specific_date = discy_options("date_recent_questions".$is_tag_dash);
			}else if ($first_one == $most_answers_slug_2) {
				$specific_date = discy_options("date_most_answered".$is_tag_dash);
			}else if ($first_one == $no_answers_slug_2) {
				$specific_date = discy_options("date_no_answers".$is_tag_dash);
			}else if ($first_one == $most_visit_slug_2) {
				$specific_date = discy_options("date_most_visited".$is_tag_dash);
			}else if ($first_one == $most_vote_slug_2) {
				$specific_date = discy_options("date_most_voted".$is_tag_dash);
			}else if ($first_one == $random_slug_2) {
				$specific_date = discy_options("date_random_questions".$is_tag_dash);
			}else if ($first_one == $question_new_slug_2) {
				$specific_date = discy_options("date_new_questions".$is_tag_dash);
			}else if ($first_one == $question_sticky_slug_2) {
				$specific_date = discy_options("date_sticky_questions".$is_tag_dash);
			}else if ($first_one == $question_polls_slug_2) {
				$specific_date = discy_options("date_poll_questions".$is_tag_dash);
			}else if ($first_one == $question_followed_slug_2) {
				$specific_date = discy_options("date_followed_questions".$is_tag_dash);
			}else if ($first_one == $question_favorites_slug_2) {
				$specific_date = discy_options("date_favorites_questions".$is_tag_dash);
			}else if ($first_one == $recent_posts_slug_2) {
				$specific_date = discy_options("date_recent_posts".$is_tag_dash);
			}else if ($first_one == $posts_visited_slug_2) {
				$specific_date = discy_options("date_posts_visited".$is_tag_dash);
			}else if ($first_one == $question_bump_slug_2) {
				$specific_date = discy_options("date_question_bump".$is_tag_dash);
			}else if ($first_one == $answers_slug_2) {
				$specific_date = discy_options("date_answers".$is_tag_dash);
			}
			$orderby_answers = discy_options("orderby_answers".$is_tag_dash);
		}else {
			if ($first_one == $feed_slug_2) {
				...
				$specific_date = discy_post_meta("date_poll_feed");
			}
		}
	}
...
if (isset($orderby_post)) {
	if ($orderby_post == "popular") {
		...
		$post_meta_stats = (has_wpqa() && wpqa_plugin_version >= "5.4"?wpqa_get_meta_stats():"post_stats");
...
$theme_sidebar = $theme_sidebar_all = discy_post_meta("sidebar");
if ($theme_sidebar == "default") {
	$theme_sidebar = $theme_sidebar_all = discy_options("sidebar_layout");
}else {
