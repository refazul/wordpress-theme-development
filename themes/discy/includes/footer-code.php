						...
						if ($sidebar_where == "menu_sidebar" || $sidebar_where == "sidebar") {?>
							...
							<aside class="sidebar<?php echo ($footer_style == "sidebar"?" footer-sidebar":"")?> sidebar-width float_l<?php echo ($widget_icons != "on"?" no-widget-icons":"").($sticky_sidebar == "sidebar" || $sticky_sidebar == "side_menu_bar"?" fixed-sidebar":"")?>">
								...
								<div class="inner-sidebar">
									<?php if (has_wpqa() && wpqa_is_user_profile()) {
										$author_widget = discy_options("author_widget");
										if ($author_widget == "on") {
						...
						<nav class="nav_menu float_r<?php echo ($sticky_sidebar == "nav_menu" || $sticky_sidebar == "side_menu_bar"?" fixed_nav_menu":"").($left_menu_style == "style_2"?" nav_menu_2":"").($left_menu_style == "style_3"?" nav_menu_3":"")?>">
							...
							if ($left_menu == true) {
								if ($tabs_menu === "on") {
									...
									$get_home_tabs = discy_post_meta("home_tabs",$home_page_id);
									$first_one = discy_home_setting($get_home_tabs);
									if (isset($get_home_tabs) && is_array($get_home_tabs)) {
										if (isset($first_one) && $first_one != "") {?>
											<ul>
												<?php discy_home_tabs($get_home_tabs,$first_one,"",$home_page_id);?>
											</ul>
										<?php }
									}
								}else {
							...
							if (has_wpqa() && wpqa_plugin_version >= "5.8" && isset($after_content_adv_filter) && $after_content_adv_filter == true) {
...
if ($skin_switcher == 'on') {
	if (is_user_logged_in()) {
		...
		$uniqid_cookie = discy_options('uniqid_cookie');
	...
	<div class="dark-light-switcher dark-light-switcher-2 <?php echo ($get_dark == 'dark'?'light-switcher':'dark-switcher')?>"><input type="checkbox" class="switcher-dark<?php echo ($get_dark == 'dark'?'':' switcher-dark-checked')?>"><span></span></div>
<?php }?>