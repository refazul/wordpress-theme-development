 ...
 * $image = discy_resize( $thumb, '', 140, 110, true );
 * ?>
 *
 * @param int $attach_id
 ...
 * @return array
 */
if ( ! function_exists( 'discy_resize' ) ) :

	function discy_resize( $attach_id = null, $img_url = null, $width = 0, $height = 0, $crop = false , $gif = false , $retina = false ) {
		global  $blog_id;
		$img_url_o = $img_url;
		$divider=1;
		if ($retina==true)
		...
		// this is not an attachment, let's use the image url
			...
			$file_path = apply_filters('wpqa_server','DOCUMENT_ROOT') . $file_path['path'];
...
function discy_image_crop_dimensions($default, $orig_w, $orig_h, $new_w, $new_h, $crop){
...
add_filter('image_resize_dimensions', 'discy_image_crop_dimensions', 10, 6);?>