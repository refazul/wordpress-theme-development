	...
	class ElitePack_Theme_Updater_Admin {
		...
		function get_download_link(){
			...
			if( ! is_wp_error( $response ) && ! empty( $response['package'] ) && empty( $response['package_error'] ) ){
				...
				if ( false !== strrpos( $response['package'], 'envato.com' ) || false !== strrpos( $response['package'], 'marketplace-downloads.customer.envatousercontent.com' ) ) {
		...
		function activate_license(){
			...
			if( isset( $_GET['tie-envato-authorize'] ) && isset( $_GET['item'] ) && $_GET['item'] == $this->item_id ){
				...
				if( isset( $_GET['sucess'] ) && ! empty( $_GET['token'] ) ){
					...
					if( is_wp_error( $response ) ){
						...
						update_option( $this->license_db_key, trim( $_GET['token'] ), false );
				...
				elseif( isset( $_GET['fail'] ) && isset( $_GET['error-description'] ) ){
					$error = $_GET['error-description'];
				}

				// If There is an Error, store the error and display the error notice
				if( ! empty( $error ) ){

					set_transient( $this->connect_error_key, esc_html( $error ), DAY_IN_SECONDS );
					add_action( 'admin_notices', array( $this, 'notice_error' ), 5 );
		 ...
		 */
		function is_active(){
			if( get_option( $this->license_status_key ) == 'valid' && get_option( $this->license_db_key ) ){
				return true;
			}
			return false;
		}
		...
		function support_period_info(){
			...
			if( ! $this->is_active() || ! $this->get( 'supported_until' ) ){
				return false;
			}

			$support_info    = array();
			$today_date      = time();
			$supported_until = $this->get( 'supported_until' );
			...
			if( $supported_until >= $today_date ){
				...
				if( $diff <= apply_filters( "{$this->prefix}/ep/expiring_duration", MONTH_IN_SECONDS ) ){
					$support_info['status'] = 'expiring';
				}
		...
		function notice_error(){
			...
			// No Errors stored
			if( false === ( $error = get_transient( $this->connect_error_key ) ) ){
				return;
			}
		 ...
		 * @since 1.0.0
		 */
		function notice( $id = false, $title = false, $message = false, $args = array() ){
						...
						<?php
							...
							foreach ( $reasons as $reason_id => $the_reason ) {
								...
								<li>
									...
									$description = ($this->is_support_active() ? (isset($the_reason['desc_active_support'])?$the_reason['desc_active_support']:"") : (isset($the_reason['desc_expired_support'])?$the_reason['desc_expired_support']:''));
