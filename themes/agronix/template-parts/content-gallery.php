...
if ( is_single() ): ?>
    ...
    <article id="post-<?php the_ID();?>" <?php post_class( 'single-news mb-55 format-gallery' );?>>
    ...
    <article id="post-<?php the_ID();?>" <?php post_class( 'single-news mb-55 format-gallery' );?> data-wow-delay=".3s">
        <?php if ( !empty($gallery_images)) : ?>
        <div class="news-thumb news-thumb-slider-active owl-carousel position-relative">
           <?php foreach( $gallery_images as $image_id ): ?>     
               <div class="single-item">
        ...
        <div class="news-detalis-content news-detalis-content-2">
            <ul class="blog-meta mb-20">
               ...
               <?php if ( !empty($agronix_blog_comments) ): ?>
               <li><a href="#"><i class="fal fa-comments"></i> <?php comments_number();?></a></li>
               <?php endif;?>

               <?php if ( !empty($agronix_blog_date) ): ?>
               <li><a href="#"><i class="fal fa-calendar-alt"></i> <?php the_time( get_option('date_format') ); ?></a></li>
               <?php endif;?>
            </ul>
