 ...
 */
    ...
    $agronix_video_url = function_exists( 'get_field' ) ? get_field( 'fromate_style' ) : NULL;
...
?>
    ...
    <article id="post-<?php the_ID();?>" <?php post_class( 'single-news mb-55 format-video' );?>>
        <?php if ( has_post_thumbnail() ): ?>    
        <div class="news-thumb position-relative">
           <a href="<?php the_permalink();?>"><?php the_post_thumbnail( 'full', ['class' => 'img-responsive'] );?></a>
            <?php if(!empty($agronix_video_url)) : ?>
           <a href="<?php print esc_url( $agronix_video_url );?>" class="play-icon play-icon-ab popup-video"><i class="fas fa-play"></i></a>
            <?php endif; ?>
        </div>
        <?php endif; ?>
...
<?php
    ...
    <article id="post-<?php the_ID();?>" <?php post_class( 'single-news mb-55 format-video' );?> data-wow-delay=".3s">
        <?php if ( has_post_thumbnail() ): ?>    
        <div class="news-thumb position-relative">
           <a href="<?php the_permalink();?>"><?php the_post_thumbnail( 'full', ['class' => 'img-responsive'] );?></a>
            <?php if(!empty($agronix_video_url)) : ?>
