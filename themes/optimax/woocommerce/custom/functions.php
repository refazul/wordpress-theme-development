...
class WC_Functions {
	...
	public function __construct() {
		...
		add_action( 'loop_shop_per_page',                  array( $this, 'optimax_loop_shop_per_page' ), 20 );
		add_action( 'woocommerce_after_shop_loop',         array( $this, 'optimax_products_paginations' ), 10 );
		// Columns
		add_filter( 'loop_shop_columns',                    array( $this, 'loop_shop_columns' ) );
		add_filter( 'woocommerce_get_image_size_thumbnail', array( $this, 'optimax_product_imgae_size' ) );
		add_filter( 'woocommerce_get_image_size_single', array( $this, 'optimax_single_product_imgae_size' ) );
		add_filter( 'woocommerce_get_image_size_gallery_thumbnail', array( $this, 'optimax_product_thumbnail_imgae_size' ) );
		...
		if ( function_exists( 'YITH_WCWL_Frontend' ) && class_exists( 'YITH_WCWL_Ajax_Handler' )  ) {
			...
			add_action( 'wp_ajax_optimax_add_to_wishlist',                array( $this, 'add_to_wishlist' ) );
			add_action( 'wp_ajax_nopriv_optimax_add_to_wishlist',         array( $this, 'add_to_wishlist' ) );
		}
		...
		/* ====== Checkout Page ====== */
		add_filter( 'woocommerce_checkout_fields', array( $this, 'optimax_checkout_fields' ) );
	}
	...
	public function optimax_product_imgae_size( $size ) {
		return array(
			'width' => 370,
			'height' => 450,
			'crop' => 1,
	...
	public function optimax_product_thumbnail_imgae_size( $size ) {
		return array(
			'width' => 155,
			'height' => 157,
			'crop' => 0,
	...
	public function optimax_single_product_imgae_size( $size ) {
		return array(
			'width' => 450,
			'height' => 515,
			'crop' => 1,
	...
	public function optimax_products_paginations(){
		Helper::pagination();
	}
	...
	public function single_socials_share(){
		...
		self::optimax_product_social_share();
		echo '</div>';
	}	
    ...
    public function optimax_loop_shop_per_page( $products ) {
	...
	public static function optimax_product_social_share(){
		...
		<div class="post-share-btn">
			<h5 class="item-label"><?php esc_html_e( 'Share:', 'optimax' );?></h5>
	...
	public static function print_quickview_icon( $icon = true, $text = false ){
		...
		if ( $text ) {
			$html .= '<span>' . esc_html__( 'QuickView', 'optimax' ) . '</span>';
		}

		?>
		<a href="#" class="yith-wcqv-button" data-product_id="<?php echo esc_attr( $product->get_id() );?>" title="<?php esc_attr_e( 'QuickView', 'optimax' ); ?>"><?php echo wp_kses_post( $html ); ?></a>
		<?php
	}
	...
	public static function print_compare_icon( $icon = true, $text = true ){
		...
		?>
		<a href="<?php echo esc_url( $url );?>" class="compare" data-product_id="<?php echo esc_attr( $id );?>" title="<?php esc_attr_e( 'Add To Compare', 'optimax' ); ?>"><?php echo wp_kses_post( $html ); ?></a>
		<?php
	}
	...
	public static function get_stock_status() {
		global $product;
		return $product->is_in_stock() ? esc_html__( 'In Stock', 'optimax' ) : esc_html__( 'Out of Stock', 'optimax' );
	}

	// WooCommerce Checkout Fields Hook
    public function optimax_checkout_fields( $fields ) {
