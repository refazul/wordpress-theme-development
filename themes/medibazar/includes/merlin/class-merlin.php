...
class Merlin {
	...
	function required_classes() {
		...
		require_once trailingslashit( $this->base_path ) . $this->directory . '/includes/class-merlin-option-tree-importer.php';
	...
	protected function plugins() {
        if(empty(medibazar_get_registered_purchase_code())) {
            echo "<p>".esc_html__('Please verify the purchase code first.','medibazar')."</p>";
	...
	public function _ajax_activate_license() {
		...
		update_option( 'envato_purchase_code_28912975', $license_key );
	...
	protected function get_import_data( $selected_import_index = 0 ) {
		...
		if ( ! empty( $import_files['option_tree'] ) ) {
			$content['option_tree'] = array(
				'title'            => esc_html__( 'Option Tree Options', 'merlin-wp' ),
				'description'      => esc_html__( 'Option Tree framework options.', 'merlin-wp' ),
				'pending'          => esc_html__( 'Pending', 'merlin-wp' ),
				'installing'       => esc_html__( 'Installing', 'merlin-wp' ),
				'success'          => esc_html__( 'Success', 'merlin-wp' ),
