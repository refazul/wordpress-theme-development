 ...
 * plugin "Breadcrumb Trail v-1.1.0"(https://wordpress.org/plugins/breadcrumb-trail/), 
...
class Breadcrumb_Class {
	...
	public function trail() {
		...
		if ( 0 < $item_count ) {
			...
			// Add the number of items and item list order schema. //@kowsar
		...
		$tags = array( 'nav', 'ul', 'li', 'a', 'span', 'meta' );
		$attributes = array( 'aria-label', 'itemprop', 'itemscope', 'itemtype', 'name', 'content' );

		foreach ( $tags as $tag ) {
			foreach ( $attributes as $attribute ) {
				$allowed_tags[$tag][$attribute] = true;
			}
		}

		echo wp_kses( $breadcrumb, $allowed_tags );
	}
	...
	protected function set_labels() {
		...
		$defaults = array(
			'browse'              => esc_html__( 'Browse:',                               'medidove' ),
			'aria_label'          => esc_attr_x( 'Breadcrumbs', 'breadcrumbs aria label', 'medidove' ),
			'home'                => esc_html__( 'Home',                                  'medidove' ),
			'error_404'           => esc_html__( '404 Not Found',                         'medidove' ),
			'archives'            => esc_html__( 'Archives',                              'medidove' ),
			// Translators: %s is the search query.
			'search'              => esc_html__( 'Search results for: %s',                'medidove' ),
			// Translators: %s is the page number.
			'paged'               => esc_html__( 'Page %s',                               'medidove' ),
			// Translators: %s is the page number.
			'paged_comments'      => esc_html__( 'Comment Page %s',                       'medidove' ),
			// Translators: Minute archive title. %s is the minute time format.
			'archive_minute'      => esc_html__( 'Minute %s',                             'medidove' ),
			// Translators: Weekly archive title. %s is the week date format.
			'archive_week'        => esc_html__( 'Week %s',                               'medidove' ),
	...
	protected function add_minute_hour_archive_items() {
		...
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute_hour'], get_the_time( esc_html_x( 'g:i a', 'minute and hour archives time format', 'medidove' ) ) );
	}
	...
	protected function add_minute_archive_items() {
		...
		// Add the minute item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_minute'], get_the_time( esc_html_x( 'i', 'minute archives time format', 'medidove' ) ) );
	}
	...
	protected function add_hour_archive_items() {
		...
		// Add the hour item.
		if ( true === $this->args['show_title'] )
			$this->items[] = sprintf( $this->labels['archive_hour'], get_the_time( esc_html_x( 'g a', 'hour archives time format', 'medidove' ) ) );
	}
	...
	protected function add_day_archive_items() {
		...
		$year  = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'medidove' ) ) );
		$month = sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'medidove' ) ) );
		$day   = sprintf( $this->labels['archive_day'],   get_the_time( esc_html_x( 'j', 'daily archives date format',   'medidove' ) ) );
	...
	protected function add_week_archive_items() {
		...
		$year = sprintf( $this->labels['archive_year'],  get_the_time( esc_html_x( 'Y', 'yearly archives date format', 'medidove' ) ) );
		$week = sprintf( $this->labels['archive_week'],  get_the_time( esc_html_x( 'W', 'weekly archives date format', 'medidove' ) ) );
	...
	protected function add_post_terms( $post_id, $taxonomy ) {
		...
		if ( $terms && ! is_wp_error( $terms ) ) {
			...
			if ( function_exists( 'wp_list_sort' ) )
				...
				$terms = wp_list_sort( $terms, 'parent', 'DESC' ); // @kowsar
	...
	protected function map_rewrite_tags( $post_id, $path ) {
		...
		if ( is_array( $matches ) ) {
			...
			foreach ( $matches as $match ) {
				...
				if ( '%year%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_year_link( get_the_time( 'Y', $post_id ) ) ), sprintf( $this->labels['archive_year'], get_the_time( esc_html_x( 'Y', 'yearly archives date format',  'medidove' ) ) ) );

				// If using the %monthnum% tag, add a link to the monthly archive.
				elseif ( '%monthnum%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_month_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ) ) ), sprintf( $this->labels['archive_month'], get_the_time( esc_html_x( 'F', 'monthly archives date format', 'medidove' ) ) ) );

				// If using the %day% tag, add a link to the daily archive.
				elseif ( '%day%' == $tag )
					$this->items[] = sprintf( '<a href="%s">%s</a>', esc_url( get_day_link( get_the_time( 'Y', $post_id ), get_the_time( 'm', $post_id ), get_the_time( 'd', $post_id ) ) ), sprintf( $this->labels['archive_day'], get_the_time( esc_html_x( 'j', 'daily archives date format', 'medidove' ) ) ) );
