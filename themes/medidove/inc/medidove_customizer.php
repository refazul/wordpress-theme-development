...
class medidove_customizer {
	...
	public function __construct($data) {
		$this->data = $data;
		add_action('customize_register',array($this,'medidove_customize_register'));
	...
	function custom_admin_style($hook) {
		if ( 'customize.php' == $hook || 'widgets.php' == $hook ) {
			wp_enqueue_style( 'medidove-customizer-style', get_template_directory_uri() . '/inc/css/medidove-customizer-style.css',array());
			wp_enqueue_script( 'medidove-customizer', get_template_directory_uri() . '/inc/js/medidove-customizer.js', array( 'jquery' ), '20150611', true );
		}
	}	
	public function medidove_customize_register($customizeObj) {
	...
	public function create_control($value,$key) {
		...
		if ( isset($value['type']) ) {
			...
			switch($value['type']){
				...
				case "switch":
					$args['type'] = $value['type'];
					$inst = new medidove_Customize_Switch_Control($this->customizeObj,$id,$args);
					break;
				case "image":
	...
	public static function medidove_data_load() {

		$data = apply_filters( 'medidove_customizer_data', array());
...
add_action('init','medidove_customizer::medidove_data_load');
...
if(class_exists( 'WP_Customize_control')) {
	...
	class medidove_Customize_Switch_Control extends WP_Customize_Control {
		public $type = 'switch';
		public function render_content() {
			...
			<label>
				...
				<div class="switch_options">
					<span class="switch_enable"><?php esc_html_e('Yes','medidove'); ?></span>
					<span class="switch_disable"><?php esc_html_e('No','medidove'); ?></span>  
