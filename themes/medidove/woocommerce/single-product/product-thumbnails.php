...
if ( count($attachment_ids) > 1):
	$markup .= '<div class="shop-thumb-tab">';
		$markup .= '<ul class="nav custom-tab" id="myTab2" role="tablist">';
			foreach ( $attachment_ids as $attachment_id ){
				...
				if( !empty($image_attributes[0]) ){
					...
					$markup .= '<li class="nav-item">';
						$markup .= '<a class="nav-link '.$class.'" id="'.$id.'-tab" data-toggle="tab" href="#'.$id.'" role="tab" aria-controls="'.$id.'" aria-selected="true"><img class="medidove-thumb" src="'.current($image_attributes).'" alt="'.esc_attr__('image','medidove').'" class="img-fluid"></a>';
					$markup .= '</li>';
					$i++;
				}
			}
