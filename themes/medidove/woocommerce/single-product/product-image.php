...
<?php
	...
	foreach ( $attachment_ids as $attachment_id ){
		...
		if( !empty($image_attributes[0]) ){
			...
			$html .=  '<div class="tab-pane fade '.$class.'" id="'.$id.'">';
				$html .= '<img src="'.$image_attributes[0].'" alt="'.esc_attr__('img','medidove').'" class="img-fluid">';
			$html .= '</div>';
			$i++;
		}
	}
	...
	if ( count($attachment_ids) <= 1){
		$tab_class = 'mb-50 tab-content-width';
	}
?>


<div class="tab-content custom-content <?php echo esc_attr($tab_class); ?>" id="myTabContent3">
	<?php echo apply_filters( 'woocommerce_single_product_image_thumbnail_html', $html, $post_thumbnail_id ); ?>
</div>

<?php do_action( 'medidove_woocommerce_product_thumbnails' ); ?>
