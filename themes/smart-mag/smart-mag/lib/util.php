 ...
 * @param int       $minimum  Any less than this and original number is returned formatted.
 * @return int|float
 */
function readable_number($number, $minimum = 1000)
{
	global $wp_locale;

	if ($number < $minimum) {
		$formatted = number_format_i18n($number, 1);
		$unit = '';
	}
	else if ($number < 10^6) {
		$formatted = number_format_i18n($number / 1000, 1);
		$unit = 'K';
	}
	else {
		$formatted = number_format_i18n($number / 10^6, 1);
		$unit = 'M';
	}

	// Remove .0 in values as it'd result in 1.0K. 
	$decimal   = isset($wp_locale) ? $wp_locale->number_format['decimal_point'] : '.';
	$formatted = rtrim($formatted, '0' . $decimal);
