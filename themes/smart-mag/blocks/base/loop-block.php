...
class LoopBlock extends Block
{
	...
	public function process()
	{
		...
		if ($this->props['query'] instanceof WP_Query) {
			...
			/**
			 * Re-execute the global query with added vars, if needed.
			 * 
			 * Note: We use the same filter in Query::setup(), so to remain consistent
			 * do the same when the query is pre-provided.
			 */ 
			...
			// Loose comparison to see if args have changed. Re-execute if needed.
			if ($query->query_vars != $new_args) {
