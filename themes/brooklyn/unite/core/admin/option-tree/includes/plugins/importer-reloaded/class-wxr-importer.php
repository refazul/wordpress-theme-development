...
class WXR_Importer extends WP_Importer {
	...
	protected function get_reader( $file ) {
		...
		// try local file reading
		$status = $reader->open( 'file://'  . ABSPATH . $file );
		
		// local file fails, try web URL reading
		if ( ! $status ) {
			$status = $reader->open( get_site_url() . $file );
		}
			...
			switch ( $reader->name ) {
				case 'wp:wxr_version':
					...
					if ( version_compare( $this->version, self::MAX_WXR_VERSION, '>' ) ) {
						...
						if(  method_exists( 'WP_Importer_Logger', 'warning' )) {
	...
	protected function log_error( WP_Error $error ) {
		...
		if ( ! empty( $data ) ) {
			...
			if(  method_exists( 'WP_Importer_Logger', 'debug' )) {
				...
				case 'wp:status':
					...
					if ( $data['post_status'] === 'auto-draft' ) {
						...
						return new WP_Error(
							...
							__( 'Cannot import auto-draft posts', 'unitedthemes' ),
							$data
						);
					}
					break;
	...
	protected function process_post( $data, $meta, $comments, $terms ) {
		...
		if ( $post_exists ) {
			...
			if(  method_exists( 'WP_Importer_Logger', 'info' )) {
		...
		if ( 'attachment' === $postdata['post_type'] ) {
			if ( ! $this->options['fetch_attachments'] ) {
				...
				if(  method_exists( 'WP_Importer_Logger', 'notice' )) {

					$this->logger->notice( sprintf(
						__( 'Skipping attachment "%s", fetching attachments disabled', 'unitedthemes' ),
						$data['post_title']
					) );
		...
		if ( is_wp_error( $post_id ) ) {
			...
			if(  method_exists( 'WP_Importer_Logger', 'error' )) {
