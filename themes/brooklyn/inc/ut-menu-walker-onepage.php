	...
	function start_el( &$output, $item, $depth = 0, $args = array() , $current_object_id = 0 ) {
	    ...
	    <li id="menu-item-<?php echo $item_id; ?>" class="<?php echo implode(' ', $classes ); ?>">
	        ...
	        <div class="menu-item-settings wp-clearfix" id="menu-item-settings-<?php echo $item_id; ?>">
                ...
                if( $item->object_id != get_option('page_for_posts') && $item->object_id != get_option('page_on_front') && ot_get_option( 'ut_site_layout', 'multisite' ) == 'onepage' ) : ?>
...
class ut_menu_walker extends Walker_Nav_Menu {
      ...
      function start_el( &$output, $item, $depth = 0, $args = array() , $current_object_id = 0 ) {
		   ...
		   if( is_front_page() && $front_page_id == $item->object_id || is_front_page() && $item->title == 'Home' )	{
           ...
           if( $depth == 0 ) {
	           $main_link = "ut-main-navigation-link";
           }
          
           $item_output  = $args->before;
           $item_output .= '<a'. $attributes .' class="' . $extraClass . ' ' . $main_link . '">';
