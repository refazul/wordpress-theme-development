...
foreach( $products as $current_product ) {
    ...
    if( $index > 0 )
        $thumbs .= '<li class="image_plus image_plus_' . $index . '" data-rel="offeringID_' . $index . '"><i class="tb-icon tb-icon-plus"></i></li>';
    ...
    <li class="yith-wfbt-item">
        <label data-for="offeringID_<?php echo esc_attr($index); ?>">
            ...
            <span class="product-name"><?php echo ! $index ? '<span>'. esc_html__( 'This item', 'diza' ) . ': </span>' . trim($current_product_title) : trim($current_product_title); ?></span>
