...
<table class="shop_table cart wishlist_table wishlist_view traditional responsive" data-pagination="<?php echo esc_attr( $pagination )?>" data-per-page="<?php echo esc_attr( $per_page )?>" data-page="<?php echo esc_attr( $current_page )?>" data-id="<?php echo esc_attr($wishlist_id); ?>" data-token="<?php echo esc_attr($wishlist_token); ?>">
    ...
    <tr>
        ...
        <th class="product-name">
            <span class="nobr">
                <?php echo apply_filters( 'yith_wcwl_wishlist_view_name_heading', esc_html__( 'Product name', 'diza' ) ) ?>
            </span>
        </th>
        ...
        <?php if( $show_price || $show_price_variations ) : $column_count ++; ?>
            <th class="product-price">
                <span class="nobr">
                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_price_heading', esc_html__( 'Unit price', 'diza' ) ) ?>
                </span>
            </th>
        <?php endif; ?>
        ...
        <?php if( $show_quantity ) : $column_count ++; ?>
            <th class="product-quantity">
                <span class="nobr">
                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_quantity_heading', esc_html__( 'Quantity', 'diza' ) ) ?>
                </span>
            </th>
        <?php endif;?>
        ...
        <?php if( $show_stock_status ) : $column_count ++; ?>
            <th class="product-stock-status">
                <span class="nobr">
                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_stock_heading', esc_html__( 'Stock status', 'diza' ) ) ?>
                </span>
            </th>
        <?php endif; ?>
        ...
        <?php if( $enable_drag_n_drop ): $column_count ++; ?>
            <th class="product-arrange">
                <span class="nobr">
                    <?php echo apply_filters( 'yith_wcwl_wishlist_view_arrange_heading', esc_html__( 'Arrange', 'diza' ) )?>
                </span>
            </th>
        <?php endif; ?>
    </tr>
    ...
    if( $wishlist && $wishlist->has_items() ) :
                ...
                <tr id="yith-wcwl-row-<?php echo esc_attr($item->get_product_id()); ?>" data-row-id="<?php echo esc_attr($item->get_product_id()); ?>">
                    ...
                    <td class="product-thumbnail">
                        ...
                        <span class="product-remove icon">
                                <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ) ?>" class="remove_from_wishlist" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', esc_html__( 'Remove this product', 'diza' ) ); ?>"><i class="tb-icon tb-icon-cross2"></i></a>
                        </span>
                        <?php endif; ?>
                    ...
                    <?php if( $show_price || $show_price_variations ) : ?>
                        <td class="product-price">
                            <?php
                            if( $show_price ) {
                                echo trim($item->get_formatted_product_price());
                            }

                            if( $show_price_variations ){
                                echo trim($item->get_price_variation());
                            }
                            ?>
                        </td>
                    <?php endif ?>
                    ...
                    <?php if( $show_stock_status ) : ?>
                        <td class="product-stock-status">
                            <?php 
                                $stock_html = ($stock_status == 'out-of-stock') ? '<span class="wishlist-out-of-stock">' . apply_filters( 'yith_wcwl_out_of_stock_label', esc_html__( 'Out of stock', 'diza' ) ) . '</span>' : '<span class="wishlist-in-stock">' . apply_filters( 'yith_wcwl_in_stock_label', esc_html__( 'In Stock', 'diza' ) ) . '</span>';
                    ...
                    <?php if( $show_last_column ): ?>
                        <td class="product-add-to-cart">
                            ...
                            if( $show_dateadded && $item->get_date_added() ):
                                echo '<span class="dateadded">' . sprintf( esc_html__( 'Added on: %s', 'diza' ), $item->get_date_added_formatted() ) . '</span>';
                            endif;
                            ?>
                            ...
                            <?php if( $move_to_another_wishlist && $available_multi_wishlist && count( $users_wishlists ) > 1 ): ?>
                                <?php if( 'select' == $move_to_another_wishlist_type ): ?>
                                    <select class="change-wishlist selectBox">
                                        <option value=""><?php _e( 'Move', 'diza' ) ?></option>
                                        <?php
                                ...
                                <?php else: ?>
                                    <a href="#move_to_another_wishlist" class="move-to-another-wishlist-button" data-rel="prettyPhoto[move_to_another_wishlist]">
                                        <?php echo apply_filters( 'yith_wcwl_move_to_another_list_label', esc_html__( 'Move to another list &rsaquo;', 'diza' ) ) ?>
                                    </a>
                                <?php endif; ?>
                            <?php endif; ?>
                    ...
                    <?php if( $repeat_remove_button ): ?>
                        <td class="product-remove">
                            <a href="<?php echo esc_url( add_query_arg( 'remove_from_wishlist', $item->get_product_id() ) ) ?>" class="remove_from_wishlist" title="<?php echo apply_filters( 'yith_wcwl_remove_product_wishlist_message_title', esc_html__( 'Remove this product', 'diza' ) ); ?>"><i class="tb-icon tb-icon-trash"></i><?php esc_html_e( 'Remove', 'diza' ) ?></a>
                        </td>
                    <?php endif; ?>
        ...
        <tr class="no-products">
            <td colspan="<?php echo esc_attr( $column_count ) ?>" class="wishlist-empty"><?php echo apply_filters( 'yith_wcwl_no_product_to_remove_message', esc_html__( 'No products added to the wishlist', 'diza' ) ) ?></td>
        </tr>
    <?php
    endif;
