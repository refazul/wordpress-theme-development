	        ...
	        <?php if( ! $show_unchecked ) : ?>
	            <div class="yith-wfbt-images">
                    ...
                    <?php $i = 0; foreach( $products as $product ) :
                        ...
                        <?php if( $i > 0 ) : ?>
                            <li class="image_plus image_plus_<?php echo esc_attr($i); ?>" data-rel="offeringID_<?php echo esc_attr($i); ?>"><i class="tb-icon tb-icon-plus"></i></li>
                        <?php endif; ?>
			        ...
			        <ul class="yith-wfbt-items">
			            <?php $j = 0; foreach( $products as $product ) :
			                ...
			                <li class="yith-wfbt-item">
			                    <label data-for="offeringID_<?php echo esc_attr($j); ?>">
			                        ...
			                        <span class="product-name">
							            <?php echo ( ( $product_id == $main_product_id ) ? '<span>'. esc_html__( 'This item', 'diza' ) . ': &nbsp;</span>' : '' ) . sprintf( '%1$s <span class="att">%2$s</span>', $product->get_title(), wc_get_formatted_variation( $product, true ) ); ?>
			                        </span>
