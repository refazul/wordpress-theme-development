...
if ( !function_exists( 'diza_tbay_autocomplete_suggestions' ) ) {
    ...
    function diza_tbay_autocomplete_suggestions() {  
		...
		if ( isset($_REQUEST['post_type']) && $_REQUEST['post_type'] == 'product') {
			if ( apply_filters( 'diza_search_query_in', diza_tbay_get_global_config('search_query_in', 'title') === 'all' ) ) {
                add_filter( 'posts_search', 'diza_product_ajax_search_sku', 9 );
            } else {
                add_filter('posts_search', 'diza_product_search_title', 20, 2);
            }
        } 
	        ...
	        while( $results->have_posts() ) {
				...
				if( $post_type == 'product' ) {
					...
					$suggestions[] = array(
						...
						'sku' => ( diza_tbay_get_global_config('search_query_in', 'title') === 'all' && diza_tbay_get_config('search_sku_ajax', false) && $product->get_sku() ) ? esc_html__( 'SKU:', 'diza' ) . ' ' . $product->get_sku() : '',
...
if (! function_exists('diza_get_transliterate')) {
    function diza_get_transliterate($slug) {
        $slug = urldecode($slug);
...
if (! function_exists('diza_elementor_general_widgets')) {
    function diza_elementor_general_widgets() {
        $elements = array(
            'template',  
            'heading',  
            'features', 
        ...
        return apply_filters('diza_general_elements_array', $elements );
    }
}

if (! function_exists('diza_elementor_header_widgets')) {
    function diza_elementor_header_widgets() {
		$elements = array(
            'site-logo',
            'nav-menu',
            'search-form',
        ...
        if ( diza_is_Woocommerce_activated() ) {
            array_push($elements, 'account');

            if ( !diza_catalog_mode_active() ) {
        ...
        return apply_filters('diza_header_elements_array', $elements );
    }
}

if (! function_exists('diza_elementor_woocommerce_widgets')) {
    function diza_elementor_woocommerce_widgets() {
        $elements = array(
            'products',
            'product-category',
            'product-tabs',
        ...
        return apply_filters('diza_woocommerce_elements_array', $elements );
    }
}

if (! function_exists('diza_redux_elementor_to_list_addons')) {
    function diza_redux_elementor_to_list_addons( $general_widgets ) {
