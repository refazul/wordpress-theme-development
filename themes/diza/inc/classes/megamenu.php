...
if ( !class_exists("Diza_Tbay_Nav_Menu") ) {
    class Diza_Tbay_Nav_Menu extends Walker_Nav_Menu {
        private $curItem;
       ...
       /**
        ...
        public function add_nav_class($classes, $item){
            if(in_array('current-menu-item', $classes) && !in_array('active', $classes) ){
                $classes[] = 'active'; 
            }
            return $classes;
        }
        ...
        public function start_lvl( &$output, $depth=0 ,$args = array() ) {  
            $width  = $this->curItem->tbay_width ? 'style="width:'.$this->curItem->tbay_width.'px"':"";
            $indent = str_repeat( "\t", $depth );
            $output    .= "\n$indent<ul class=\"dropdown-menu\" $width>\n";
        }
        ...
        public function start_el( &$output, $item, $depth = 0, $args = array(), $id = 0 ) {
            ...
            if ( isset($item->tbay_text_label) && !empty($item->tbay_text_label) ) {
                switch ( $item->tbay_text_label ) {
                    case 'label_new':
                        $text_label  = esc_html__('New', 'diza');
                        break;

                    case 'label_hot':
                        $text_label  = esc_html__('Hot', 'diza');
                        break;

                    case 'label_featured':
                        $text_label  = esc_html__('Sale', 'diza');
                        break;
        ...
        public function generateSubMegaMenu( $item, $tbay_mega_profile ) {
            ...
            if ( $tbay_mega_profile ) {
                ...
                if ( $posts && isset($posts[0]) ) {
                    ...
                    if( diza_is_elementor_activated() && Elementor\Plugin::instance()->documents->get( $post->ID )->is_built_with_elementor() ) {
                    ...
                    if( diza_tbay_get_config('ajax_dropdown_megamenu', false) ) {  
                    ...
                    return '<div '. $class_names .' '.$width.'><div class="dropdown-menu-inner">'. $output_content .'</div></div>';
                }
            }
            return '';
        }
