 ...
 */
if (!function_exists('diza_subtotal_free_shipping')) {
    add_action('woocommerce_widget_shopping_cart_total', 'diza_subtotal_free_shipping', 20);
    function diza_subtotal_free_shipping($return = false) {
        $content = ''; 
        ...
        if ($subtotalCart < $value) {
            ...
            sprintf(
                wp_kses(__('Spend %s more to reach <strong>FREE SHIPPING!</strong> <br /><span class="hide-in-cart">to add more products to your cart and receive free shipping for order %s.</span>', 'diza'), $allowed_html),
                wc_price($spend),
                wc_price($value)
            ) . 
            '</div>';
         ...
         */
            ...
            $content .= '<div class="tbay-total-condition-desc">';
            $content .= sprintf(
                esc_html__("Congratulations! You get free shipping with your order greater %s.", 'diza'),
                wc_price($value)
            );
            $content .= '</div>';
            $content .= '</div>';
 ...
 */
if (!function_exists('diza_subtotal_free_shipping_in_cart')) {
    add_action('woocommerce_cart_actions', 'diza_subtotal_free_shipping_in_cart', 10);
    function diza_subtotal_free_shipping_in_cart()
    {
        $content = diza_subtotal_free_shipping(true);
        if ($content !== '') {
