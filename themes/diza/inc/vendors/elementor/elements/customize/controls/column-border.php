...
if ( !function_exists('diza_column_child_border')) {
    function diza_column_child_border( $widget ) {
        if( get_post_type() !== 'tbay_header'  ) return;

        $widget->start_controls_section(
            'element_child_border',
            array(
                'label' => esc_html__( 'Element border child', 'diza' ),
        ...
        $widget->add_control(
            ...
            array(
                'label'                 =>  esc_html__( 'Enable element border child', 'diza' ),
    ...
    add_action( 'elementor/element/column/section_style/before_section_start', 'diza_column_child_border', 10, 1 );
}
