...
if ( ! function_exists( 'diza_search_title_query' ) ) {
	function diza_search_title_query( $search, $wp_query ) {
...
if ( ! function_exists( 'diza_product_search_title' ) ) {
	function diza_product_search_title( $search, $wp_query ) {

		if( !(defined('TBAY_ELEMENTOR_ACTIVED') && TBAY_ELEMENTOR_ACTIVED) ) return $search;

		$where = diza_search_title_query($search, $wp_query);

		return $where; 
	}
}

if ( ! function_exists( 'diza_product_ajax_search_sku' ) ) {
	function diza_product_ajax_search_sku( $where ) {
        if ( !defined('TBAY_ELEMENTOR_ACTIVED') ) return $where;

		if ( ! empty( $_REQUEST['query'] ) ) {
			$s = sanitize_text_field( $_REQUEST['query'] );

            return diza_sku_search_query( $where, $s );
		}
 ...
 * ------------------------------------------------------------------------------------------------
 */
if ( ! function_exists( 'diza_init_product_search' ) ) {
	function diza_init_product_search() {
        if( diza_is_Woocommerce_activated() ) {
            if ( apply_filters( 'diza_search_query_in', diza_tbay_get_global_config('search_query_in', 'title') === 'all' ) ) {
                add_filter( 'posts_search', 'diza_product_search_sku', 9 );
            } else {
                add_filter('posts_search', 'diza_product_search_title', 20, 2);
            }
        }
	}
	add_action( 'init', 'diza_init_product_search', 10 ); 
}

if ( ! function_exists( 'diza_product_search_sku' ) ) {
	function diza_product_search_sku( $where ) {
        if ( !defined('TBAY_ELEMENTOR_ACTIVED') ) return $where;
...
if ( ! function_exists( 'diza_sku_search_query' ) ) {
	function diza_sku_search_query( $where, $s ) {
		global $wpdb;
