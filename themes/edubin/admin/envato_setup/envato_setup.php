...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
    ...
    class Envato_Theme_Setup_Wizard {
        ...
        public function get_logo_image() {
            $image_url = get_theme_mod( 'logo_header_image', get_template_directory_uri() . '/assets/images/text-logo.png' );
        ...
        public function init_globals() {
            ...
            $this->envato_username = apply_filters( $this->theme_name . '_theme_setup_wizard_username', 'pixelcurve' );
            $this->oauth_script    = apply_filters( $this->theme_name . '_theme_setup_wizard_oauth_script', '//www.thepixelcurve.com/envato/api/server-script.php' );
        ...
        public function admin_menus() {
         add_submenu_page('edubin-admin-menu', esc_html__( 'Setup Wizard','edubin' ), esc_html__( 'Setup Wizard','edubin' ), 'manage_options', $this->page_slug,  array( $this, $this->page_slug) );
     }
        ...
        public function init_wizard_steps() {
            ...
            $this->steps = array(
                'introduction' => array(
                    'name'    => esc_html__( 'Introduction','edubin' ),
        ...
        public function setup_wizard() {
            ...
            wp_localize_script( 'envato-setup', 'envato_setup_params', array(
                ...
                'verify_text'      => esc_html__( '...verifying','edubin' ),
            ) );
        ...
        public function setup_wizard_header() {
            ...
            // avoid theme check issues.
                echo '<t';
                echo 'itle>' . __( 'Theme &rsaquo; Setup Wizard','edubin' ) . '</ti' . 'tle>'; ?>
            ...
            <body class="envato-setup wp-core-ui">
                <h1 id="wc-logo">
                    ...
                    if ( $image_url ) {
                        ...
                        <img src="<?php echo esc_url( $this->plugin_url . 'images/text-logo.png' ); ?>" alt="<?php esc_attr_e( 'Envato install wizard', 'edubin' );?>" /><?php
                    } ?>
                </h1>
                <?php
            }
        ...
        public function setup_wizard_footer() {
            ...
            <?php if ( 'next_steps' === $this->step ) : ?>
                ...
                href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the WordPress Dashboard','edubin' ); ?></a>
            <?php endif; ?>
        </body>
        <?php
        ...
        public function envato_setup_introduction() {
            ...
            if ( false && isset( $_REQUEST['debug'] ) ) {
                ...
                ?>
                <h1><?php printf( __( 'Welcome to the setup wizard for %s.','edubin' ), wp_get_theme() ); ?></h1>

                <p><?php esc_html_e( 'It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.','edubin' ); ?></p>
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                     class="button-primary button button-large button-next"><?php esc_html_e( 'Let\'s Go!','edubin' ); ?></a>
                     <a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
                         class="button button-large"><?php esc_html_e( 'Not right now','edubin' ); ?></a>
                     </p>
                     <?php
                 } else if ( get_option( 'envato_setup_complete', false ) ) {
                    ?>
                    <h1><?php printf( __( 'Welcome to the setup wizard for %s.','edubin' ), wp_get_theme() ); ?></h1>
                    <p><?php esc_html_e( 'It looks like you have already run the setup wizard. Below are some options: ','edubin' ); ?></p>

                    <ul>
                        <li>
                            <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                             class="button-primary button button-next button-large"><?php esc_html_e( 'Run Setup Wizard Again','edubin' ); ?></a>
                         </li>
                     </ul>
                     <p class="envato-setup-actions step">
                        <a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(), 'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
                         class="button button-large"><?php esc_html_e( 'Cancel','edubin' ); ?></a>
                     </p>
                     <?php
                 } else {
                    ?>
                    ...
                    <p><?php printf( __( 'Thank you for choosing the %s theme from ThemeForest. Just complete the steps below and you will be able to use all functionalities of Edubin theme by Pixelcurve:','edubin' ), wp_get_theme() ); ?></p>
                       <p>
                        <?php printf( __( 'If you need any help, please do not hesitate to contact our support desk. <a target="_blank" class="" href="%s">https://thepixelcurve.com/support/ </a>','edubin' ),  esc_url('https://thepixelcurve.com/support/') ); ?> 
                        </p> 
                        ...
                        <ul>
                          ...
                          <li>
                              <span class="step">
                                  <?php esc_html_e('Step 1', 'edubin');?>
                              </span>
                              <?php esc_html_e('Install Required and recommended plugins.', 'edubin');?>
                          </li>
                          <li>
                              <span class="step">
                                  <?php esc_html_e('Step 2', 'edubin');?>
                              </span>
                              <?php esc_html_e('Activate your license.', 'edubin');?>
                          </li>
                          <li>
                              <span class="step">
                                  <?php esc_html_e('Step 3', 'edubin');?>
                              </span>
                              <?php esc_html_e('Import demo content', 'edubin');?>
                          </li>
                        </ul>
        ...
        public function envato_setup_introduction_save() {
            ...
            if ( ! empty( $_POST['reset-font-defaults'] ) && $_POST['reset-font-defaults'] == 'yes' ) {
                ...
                ?>
                <p>
                    <strong><?php esc_html_e( 'Options have been reset. Please go to Appearance > Customize in the WordPress backend.','edubin' ); ?></strong>
                </p>
                <?php
                return true;
            }
        ...
        public function envato_setup_default_plugins() {
            ...
            ?>
            <h1><?php esc_html_e( 'Default Plugins','edubin' ); ?></h1>
            <form method="post">
                ...
                if ( count( $plugins['all'] ) ) {
                    ?>
                    <p><?php esc_html_e( 'Your website needs a few essential plugins. The following plugins will be installed or updated:','edubin' ); ?></p>
                ...
                echo '<p><strong>' . esc_html_e( 'Good news! All plugins are already installed and up to date. Please continue.','edubin' ) . '</strong></p>';
            } ?>

            <p><?php esc_html_e( 'You can add and remove plugins later on from within WordPress.','edubin' ); ?></p>

            <p class="envato-setup-actions step">
                <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                 class="button-primary button button-large button-next"
                 data-callback="install_plugins"><?php esc_html_e( 'Continue','edubin' ); ?></a>
                 <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                     class="button button-large button-next"><?php esc_html_e( 'Skip this step','edubin' ); ?></a>
         ...
         public function ajax_plugins() {
            if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
                wp_send_json_error( array( 'error' => 1, 'message' => __( 'No Slug Found','edubin' ) ) );
            }
            $json = array();
            ...
            foreach ( $plugins['activate'] as $slug => $plugin ) {
                if ( $_POST['slug'] == $slug ) {
                    $json = array(
                        ...
                        'action2'       => - 1,
                        'message'       => esc_html__( 'Activating Plugin','edubin' ),
                    );
                    break;
                }
            }
            foreach ( $plugins['update'] as $slug => $plugin ) {
                if ( $_POST['slug'] == $slug ) {
                    $json = array(
                        ...
                        'action2'       => - 1,
                        'message'       => esc_html__( 'Updating Plugin','edubin' ),
                    );
                    break;
                }
            }
            foreach ( $plugins['install'] as $slug => $plugin ) {
                if ( $_POST['slug'] == $slug ) {
                    $json = array(
                        ...
                        'action2'       => - 1,
                        'message'       => esc_html__( 'Installing Plugin','edubin' ),
                    );
                    break;
                }
            }
            ...
            if ( $json ) {
                ...
                wp_send_json( array( 'done' => 1, 'message' => esc_html__( 'Success','edubin' ) ) );
            }
            exit;
        ...
        private function _content_default_get() {
            ...
            foreach ( $available_content as $post_type => $post_data ) {
                if ( count( $post_data ) ) {
                    ...
                    $content[ $post_type ] = array(
                        ...
                        'description'      => sprintf( __( 'This will create default %s as seen in the demo.','edubin' ), $post_type_title ),
                        'pending'          => esc_html__( 'Pending.','edubin' ),
                        'installing'       => esc_html__( 'Installing.','edubin' ),
                        'success'          => esc_html__( 'Success.' ,'edubin'),
            ...
            $content['widgets'] = array(
                'title'            => esc_html__( 'Widgets','edubin' ),
                'description'      => esc_html__( 'Insert default sidebar widgets as seen in the demo.' ,'edubin'),
                'pending'          => esc_html__( 'Pending.' ,'edubin'),
                'installing'       => esc_html__( 'Installing Default Widgets.','edubin' ),
                'success'          => esc_html__( 'Success.','edubin' ),
            ...
            $content['settings'] = array(
                'title'            => esc_html__( 'Settings' ,'edubin'),
                'description'      => esc_html__( 'Configure default settings.' ,'edubin'),
                'pending'          => esc_html__( 'Pending.','edubin' ),
                'installing'       => esc_html__( 'Installing Default Settings.' ,'edubin'),
                'success'          => esc_html__( 'Success.','edubin' ),
        ...
        public function envato_setup_default_content() {
            ?>
            <h1><?php esc_html_e( 'Default Content','edubin' ); ?></h1>
            <form method="post">
                <?php if ( $this->is_possible_upgrade() ) { ?>
                    <p><?php esc_html_e( 'It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.','edubin' ); ?></p>
                <?php } else { ?>
                    <p><?php printf( __( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ','edubin' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
                <?php } ?>
                <table class="envato-setup-pages" cellspacing="0">
                    <thead>
                        <tr>
                            <td class="check"></td>
                            <th class="item"><?php esc_html_e( 'Item','edubin' ); ?></th>
                            <th class="description"><?php esc_html_e( 'Description','edubin' ); ?></th>
                            <th class="status"><?php esc_html_e( 'Status','edubin' ); ?></th>
                        </tr>
                    </thead>
                    <tbody>
                ...
                <p class="envato-setup-actions step">
                    <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                     class="button-primary button button-large button-next"
                     data-callback="install_content"><?php esc_html_e( 'Continue','edubin' ); ?></a>
                     <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                         class="button button-large button-next"><?php esc_html_e( 'Skip this step','edubin' ); ?></a>
                         <?php wp_nonce_field( 'envato-setup' ); ?>
                     </p>
             ...
             public function ajax_content() {
                ...
                if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
                    wp_send_json_error( array( 'error' => 1, 'message' => esc_html__( 'No content Found' ,'edubin') ) );
                }
                ...
                wp_send_json( array(
                    'error'   => 1,
                    'message' => esc_html__( 'Error','edubin' ),
                    'logs'    => $this->logs,
                    'errors'  => $this->errors,
                ) );
            }
        ...
        private function _fetch_remote_file( $url, $post ) {
            ...
            if ( ! $upload || $upload['error'] ) {
                ...
                if ( is_array( $response ) && ! empty( $response['body'] ) && $response['response']['code'] == '200' ) {
                    ...
                    return new WP_Error( 'import_file_error', esc_html__( 'Remote server did not respond','edubin' ) );
                }
                ...
                if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
                    ...
                    return new WP_Error( 'import_file_error', esc_html__( 'Remote file is incorrect size' ,'edubin') );
                }

                if ( 0 == $filesize ) {
                    @unlink( $upload['file'] );

                    return new WP_Error( 'import_file_error', esc_html__( 'Zero size file downloaded','edubin' ) );
                }

                if ( ! empty( $max_size ) && $filesize > $max_size ) {
                    @unlink( $upload['file'] );

                    return new WP_Error( 'import_file_error', sprintf( __( 'Remote file is too large, limit is %s','edubin' ), size_format( $max_size ) ) );
                }
            }
        ...
        public function envato_setup_updates() {
            ?>
            <h1><?php esc_html_e( 'Theme Updates','edubin' ); ?></h1>
            <?php if ( function_exists( 'envato_market' ) ) { ?>
                <form method="post">
                    ...
                    if ( count( $my_items ) ) {
                        ?>
                        <p><?php esc_html_e( 'Thanks! Theme updates have been enabled for the following items:','edubin' ); ?></p>
                        <ul>
                            <?php foreach ( $my_items as $item ) { ?>
                                <li><?php echo esc_html( $item['name'] ); ?></li>
                            <?php } ?>
                        </ul>
                        <p><?php esc_html_e( 'When an update becomes available it will show in the Dashboard with an option to install.','edubin' ); ?></p>
                        <p><?php esc_html_e( "Change settings from the 'Envato Market' menu in the WordPress Dashboard.",'edubin' ); ?></p>

                        <p class="envato-setup-actions step">
                            <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                             class="button button-large button-next button-primary"><?php esc_html_e( 'Continue','edubin' ); ?></a>
                         </p>
                         <?php
                     } else {
                        ?>
                        <p><?php esc_html_e( 'ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.','edubin' ); ?></p>
                        <p><?php esc_html_e( "When an update becomes available it will show in the Dashboard with an option to install.",'edubin' ); ?> <?php printf( __( 'To get it you have to configure the Envato Market Plugin.') ); ?> 
                
                                <?php printf( __( '<a target="_blank" class="" href="%s">Follow the article to configure it</a>.','edubin' ),  esc_url('https://thepixelcurve.com/how-to-update-themeforest-wordpress-theme/') ); ?> 
                        </p> 
                            <p>
                                <?php printf( __( 'If you need any help, please do not hesitate to contact our support desk. <a target="_blank" class="" href="%s">https://thepixelcurve.com/support/ </a>','edubin' ),  esc_url('https://thepixelcurve.com/support/') ); ?> 
                            </p> 
                        ...
                        <p class="envato-setup-actions step">
                            ...
                            <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                             class="button button-primary button-large button-next"><?php esc_html_e( 'Continue','edubin' ); ?></a>
             ...
             <?php } else { ?>
                <?php esc_html_e( 'Please ensure the Envato Market plugin has been installed correctly.','edubin' ); ?> <a
                href="<?php echo esc_url( $this->get_step_link( 'default_plugins' ) ); ?>"><?php esc_html_e( 'Return to Required
                Plugins installer.','edubin' ); ?></a>
            <?php } ?>
            <?php
        }
        ...
        public function envato_setup_customize() {
            ...
            <h1><?php esc_html_e( 'Theme Customization','edubin' ); ?></h1>
            <p><?php esc_html_e( 'Most changes to the website can be made through the Edubin > Theme Options menu from the WordPress
            dashboard. These include:','edubin' ); ?> 
        </p>
        <ul>
            <li><?php esc_html_e( 'Styles: Whole site global settings, typography, colors, styles, etc.','edubin' ); ?>
        </li>
        <li><?php esc_html_e( 'Variation: You can change all of the header & home page variation from here!','edubin' ); ?></li>
        <li><?php esc_html_e( 'LMS Options: You can manage LearnPress and LearnDash LMS options.','edubin' ); ?></li>
        <li><?php esc_html_e( 'Blog: Here you will get all blog related options.','edubin' ); ?></li>
        <li><?php esc_html_e( 'Plugins: You can manage WooCommerce products & The Events Calendar plugin settings.','edubin' ); ?></li>
    </ul>
    <p><?php esc_html_e( 'To change the Sidebars go to Appearance > Widgets. Here widgets can be "drag &amp; droped" into sidebars. More details in documentation.','edubin' ); ?></p>
    <p>
        <em><?php esc_html_e( 'Advanced Users: If you are going to make changes to the theme source code please use a','edubin' ); ?> <a
            href="//developer.wordpress.org/themes/advanced-topics/child-themes/" target="_blank"><?php esc_html_e( 'Child Theme','edubin' ); ?></a> <?php esc_html_e( 'rather than
                modifying the main theme HTML/CSS/PHP code. This allows the parent theme to receive updates without
                overwriting your source code changes.','edubin' ); ?> <br/> <?php esc_html_e( 'See','edubin' ); ?> <code><?php esc_html_e( 'edubin-child.zip','edubin' ); ?></code> <?php esc_html_e( 'in the main theme folder.','edubin' ); ?></em>
            </p>
         ...
         public function envato_setup_help_support() {
            ?>
            <h1><?php esc_html_e( 'Help and Support','edubin' ); ?></h1>
            <p><?php esc_html_e( 'This theme comes with 6 months item support from purchase date (with the option to extend this period).
                ...
                use this theme on another website.','edubin' ); ?></p>

                <p class="greendata"><?php esc_html_e( 'Item Support can be accessed from','edubin' ); ?> <a href="<?php echo esc_url('https://thepixelcurve.com/support'); ?>" target="_blank"><?php esc_html_e( 'ThePixelcurve Help','edubin' ); ?></a>
                 <?php esc_html_e( 'and includes:','edubin' ); ?></p>
                 <ul>
                    <li><?php esc_html_e( 'Availability of the author to answer questions','edubin' ); ?></li>
                    <li><?php esc_html_e( 'Answering technical questions about item features','edubin' ); ?></li>
                    <li><?php esc_html_e( 'Assistance with reported bugs and issues','edubin' ); ?></li>
                    <li><?php esc_html_e( 'Help with bundled 3rd party plugins','edubin' ); ?></li>
                </ul>

                <p class="reddata"><?php esc_html_e( 'Item Support DOES NOT Include:', 'edubin' ); ?></p>
                <ul>
                    <li><?php esc_html_e( 'Customization services', 'edubin' ); ?>
                </li>
               
            </li>
           
        </ul>
        <p><?php esc_html_e( 'More details about item support can be found in the ThemeForest', 'edubin' ); ?> <a
            href="https://themeforest.net/page/item_support_policy" target="_blank"><?php esc_html_e( 'Item Support Polity.', 'edubin' ); ?></a></p>

            <p class="envato-setup-actions step">
                <a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
                 class="button button-primary button-large button-next"><?php esc_html_e( 'Agree and Continue','edubin' ); ?></a>
                 <?php wp_nonce_field( 'envato-setup' ); ?>
             </p>
             <?php
         }
        ...
        public function envato_setup_ready() {
            ...
            ?>
            <h1><?php esc_html_e( 'Plugins install completed','edubin' ); ?></h1>

            <p class="greendata"><?php esc_html_e( 'Congratulations! The theme and the required plugins have been activated. Click the "Activate Your Licence" button for demo import.','edubin' ); ?></p>

                <div class="envato-setup-next-steps">
                    <div class="envato-setup-next-steps-first">
                        <h3><?php esc_html_e( 'Next Steps','edubin' ); ?></h3>
                        <ul>
                            <?php $url = admin_url().'themes.php?page=one-click-demo-import'; ?>
                            <li class="setup-product"><a class="button button-primary button-large"
                               href="<?php echo esc_url($url); ?>"><?php esc_html_e( 'Import Dummy Content', 'edubin' ); ?></a>
                           </li>
                           <li class="setup-product"><a class="button button-next button-large"
                               href="<?php echo esc_url( home_url('/') ); ?>"><?php esc_html_e( 'View your new website!','edubin' ); ?></a>
                           </li>
                           <li class="setup-product"><a class="button button-next button-large"
                               href="<?php echo esc_url( admin_url() ); ?>"><?php esc_html_e( 'Return to the Dashboard','edubin' ); ?></a>
                           </li>
                       </ul>
                   </div>
                   <div class="envato-setup-next-steps-last">
                    <h3><?php esc_html_e( 'More Resources','edubin' ); ?></h3>
                    <ul>
                        <li class="documentation"><a href="<?php echo esc_url( 'https://thepixelcurve.com/support/docs/edubin/' ); ?>"
                           target="_blank"><?php esc_html_e( 'Read the Theme Documentation','edubin'); ?></a>
                       </li>
                        <li class="support"><a href="<?php echo esc_url( 'https://thepixelcurve.com/support/' ); ?>"
                             target="_blank"><?php esc_html_e( 'Get Help and Support','edubin' ); ?></a></li>

                       <li class="howto"><a href="<?php echo esc_url('https://wordpress.org/support/' ); ?>"
                           target="_blank"><?php esc_html_e( 'Learn how to use WordPress','edubin' ); ?></a>
                       </li>

                       <li class="rating"><a href="<?php echo esc_url('https://themeforest.net/downloads' ); ?>"
                          target="_blank"><?php esc_html_e( 'Leave an Item Rating','edubin' ); ?></a></li>
            ...
            //add_thickbox();
                ...
                // request the token from our bounce url.
                    ...
                    if ( is_wp_error( $response ) ) {
                        ...
                        $class         = 'error';
                        echo "<div class=\"$class\"><p>" . sprintf( __( 'Something went wrong while trying to retrieve oauth token: %s','edubin' ), $error_message ) . '</p></div>';
                    } else {
                ...
                add_settings_section(
                    ...
                    sprintf( __( 'Login for %s updates','edubin' ), $this->envato_username ),
                ...
                if ( !$option || empty($option['oauth']) || empty( $option['oauth'][ $this->envato_username ] ) || empty($envato_items) || empty($envato_items['items']) || !envato_market()->items()->themes( 'purchased' )) {
                    ...
                    if ( ! $dissmissed_time || $dissmissed_time < strtotime('-7 days') ) {
                        ...
                        <div class="notice notice-warning notice-dtbwp-themeupdates is-dismissible">
                            ...
                            <p>
                                <?php printf( __( 'Configure the Envato Market Plugin to receive theme update notifications.') ); ?> 
